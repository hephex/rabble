...
pub trait ServiceHandler<T> {
    ...
    fn init(&mut self, &mut Registrar, &Node<T>) -> Result<()> {
        Ok(())
    }
    ...
    fn handle_notification(&mut self, &Node<T>, Notification, &mut Registrar) -> Result<()> {
        // TODO: Log message
        Ok(())
    }
    ...
    /// this function.
    fn handle_envelope(&mut self, &Node<T>, Envelope<T>, &mut Registrar) -> Result<()>;
}
